...
function read_manifest_versions() {
    let core_path = srcFolder + 'core/manifest.js'
    let custom_path = srcFolder + 'custom/core/manifest.js'

    // Спочатку читаємо базову версію з core/manifest.js
    if (!fs.existsSync(core_path)) {
        throw new Error('Core manifest not found: ' + core_path)
    }

    let core_content = fs.readFileSync(core_path, 'utf8')
    let core_app_match = core_content.match(/app_version:\s*'([^']+)'/)
    let core_css_match = core_content.match(/css_version:\s*'([^']+)'/)

    if (!core_app_match || !core_css_match) {
        throw new Error('Unable to read base version from core manifest')
    }

    let app_version = core_app_match[1]
    let css_version = core_css_match[1]
    let source = core_path

    // Якщо є кастомний manifest з patch_version — додаємо його
    if (fs.existsSync(custom_path)) {
        let custom_content = fs.readFileSync(custom_path, 'utf8')
        let patch_match = custom_content.match(/patch_version:\s*(\d+)/)

        if (patch_match) {
            let patch = parseInt(patch_match[1])
            if (patch >= 1 && patch <= 100) {
                app_version = app_version + '.' + patch
                css_version = css_version + '.' + patch
                source = custom_path
            }
        }
    }
